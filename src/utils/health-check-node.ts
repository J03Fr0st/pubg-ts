...
export class HealthChecker {
  ...
  private async checkMemory(): Promise<HealthCheckResult> {
    ...
    if (typeof process !== 'undefined') {
      ...
      memoryPercentage = (memStats.heapUsed / memStats.heapTotal) * 100;
  ...
  private async checkProcessHealth(): Promise<HealthCheckResult> {
    ...
    try {
      // Browser-compatible process health check
      let status: 'pass' | 'fail' | 'warn' = 'pass';
      let output = 'Process health check passed';
      let observedValue = 1.0;
      ...
      if (typeof process !== 'undefined') {
        ...
        output = 'Browser environment - process health OK';
      }
      ...
      return {
        ...
        componentType: 'system',
        observedValue,
        observedUnit: typeof process !== 'undefined' ? 'load_average' : 'status',
        output,
        time: new Date().toISOString(),
        duration: performance.now() - startTime,
      };
