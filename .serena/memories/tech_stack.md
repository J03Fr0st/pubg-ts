...
- HTTP: axios with a custom `HttpClient` wrapper; consumers should not bypass it for PUBG API endpoints unless the endpoint is explicitly external telemetry URL handling.
...
- README/AGENTS mention `sync-assets`, security, perf, health, CLI, and scripts directories, but current `package.json` does not define those commands and current root has no `scripts/` or `src/cli/`. Verify the actual tree before relying on those instructions.
