...
- `src/api/client.ts` exposes `PubgClient`; each instance owns one internal `ClientRuntime`, public `players`, `matches`, `seasons`, `leaderboards`, `samples`, and `assets` members, plus synchronous `getHealth()` and `clearResponseCache()` methods. Match telemetry is fetched through `matches.getTelemetry()`.
