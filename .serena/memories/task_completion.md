...
- For `HttpClient`, cache, rate limiter, error, monitoring, security, or asset manager changes: run the matching unit test file(s), then `npm run test:unit`; broader `npm test` if behavior may affect integration tests.
