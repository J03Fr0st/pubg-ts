...
- 💾 **Client-local response caching** with TTL and size limits
...
- ❤️ **Synchronous Client Health** derived from real request outcomes
- 📦 **Local-only Asset Catalog** with generated image URLs
...
Upgrading from v1? See [MIGRATION.md](./MIGRATION.md) for the breaking interface changes.
...
`AssetCatalog` always reads bundled local data synchronously. `assetBaseUrl` only controls the
image URLs generated by the catalog.

```typescript
import { AssetCatalog, ItemId, VehicleId } from '@j03fr0st/pubg-ts';

// Every client owns a local catalog.
const itemId: ItemId = 'Item_Weapon_AK47_C';
const itemName = client.assets.getItemName(itemId); // "AKM"
const itemInfo = client.assets.getItemInfo(itemId);
...
// Standalone catalogs use the same local data and can customize generated image URLs.
...
Each client caches eligible API responses locally:

```typescript
const health = client.getHealth();
console.log(
  `Cache: ${health.responseCache.size}/${health.responseCache.maxSize} entries`
);
...
Inspect the last-known rate-limit state through Client Health:

```typescript
const health = client.getHealth();
console.log(`Remaining requests: ${health.rateLimit.remaining}`);
console.log(`Reset time: ${health.rateLimit.resetAt ?? 'not observed'}`);
```
...
This project uses Lefthook for pre-commit validation:
